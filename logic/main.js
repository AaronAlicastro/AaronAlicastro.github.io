...
document.onscroll = () => {
  ...
  let allScroll =
    Math.floor((window.innerHeight / html.scrollHeight) * 100) +
    Math.floor((html.scrollTop / html.scrollHeight) * 100);
...
page_float_button.f("event", "click", () =>
...
aplicarPara(considera(".button_view_project", 0), "event", "click", (e) => {
  ...
  // poniendo titulo
  project_description_des[0].f(
    "innerText",
    languague_switch.my_projects.currentLanguague[index].title
  );
  // poniendo descripción
  project_description_des[1].f(
    "html",
    languague_switch.my_projects.currentLanguague[index].long_description
  );
  // poniendo boton 1 (ver pagina)
  if (
    languague_switch.my_projects.currentLanguague[index].first_button.enable
  ) {
    project_description_des[2].f("display", "inline-block");
    project_description_des[2].f(
      "innerText",
      languague_switch.my_projects.currentLanguague[index].first_button.tag
    );
    project_description_des[2].e.onclick = () =>
      window.open(
        languague_switch.my_projects.currentLanguague[index].first_button.link
      );
  } else project_description_des[2].f("display", "none");
  // poniendo boton 2 (ver codigo)
  if (
    languague_switch.my_projects.currentLanguague[index].second_button.enable
  ) {
    project_description_des[3].f("display", "inline-block");
    project_description_des[3].f(
      "innerText",
      languague_switch.my_projects.currentLanguague[index].second_button.tag
    );
    project_description_des[3].e.onclick = () =>
      window.open(
        languague_switch.my_projects.currentLanguague[index].second_button.link
      );
...
game_introduccion[1].e.onclick = (e) => {
  if (!game_introduccion[0].gameSelected) {
    ...
    languague_switch.typed(
      game.description,
      (character) => game_introduccion[0].f("html+", character),
      () => {
        game_introduccion[0].f(
          "apend",
          span_reject_current_game(languague_switch.englishIsOn, () => {
            game_introduccion[0].gameSelected = false;
            game_introduccion[0].f("html", "");
            game_introduccion[1].f(
              "html",
              languague_switch.my_games.currentLanguague.button
            );
            languague_switch.typed(
              languague_switch.my_games.currentLanguague.title,
              (character) => game_introduccion[0].f("html+", character),
              () => {}
            );
          })
...
aplicarPara(considera(".img_link_contact", 0), "event", "click", (e) => {
  if (e.target.name == "phone_broken") {
    navigator.clipboard
      .writeText(PAGE_VARIABLES.phone_number)
      .then(() => {
    ...
    navigator.clipboard
      .writeText(PAGE_VARIABLES.email)
      .then(() => {
        window.alert("¡Copiado!");
      })
      .catch((err) => console.log(err));
